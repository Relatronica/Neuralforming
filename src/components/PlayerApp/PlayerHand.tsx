...
import { Plus, Hand, ScrollText, Brain, Scale, Microscope } from 'lucide-react';
...
export const PlayerHand: React.FC<PlayerHandProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'hand' | 'laws'>('hand');
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-2xl p-4 sm:p-6 mb-4">
          <div className="flex items-center gap-2 mb-3 sm:mb-4">
            {activeTab === 'hand' ? (
              <Hand className="w-5 h-5 sm:w-6 sm:h-6 text-gray-800 flex-shrink-0" />
            ) : (
              <ScrollText className="w-5 h-5 sm:w-6 sm:h-6 text-gray-800 flex-shrink-0" />
            )}
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">
              {activeTab === 'hand' ? 'Le Tue Proposte' : 'Leggi Approvate'}
            </h1>
          </div>
          ...
          <>
            {player.hand.length === 0 ? (
              <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 text-center">
                <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">Non hai proposte disponibili</p>
                <button
                  onClick={onDrawTechnology}
                  className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3 sm:py-4 px-6 rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg active:shadow-md flex items-center justify-center gap-2 text-sm sm:text-base"
                >
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                {player.hand.map((tech) => (
                  <div key={tech.id} className="bg-white rounded-xl shadow-lg p-3 sm:p-4 hover:shadow-xl transition-shadow duration-200">
                    <TechnologyCard
                      technology={tech}
                      onSelect={(tech) => {
                ...
                <button
                  onClick={onDrawTechnology}
                  className="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold py-3 sm:py-4 px-6 rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg active:shadow-md flex items-center justify-center gap-2 text-sm sm:text-base mt-4"
                >
                  <Plus className="w-5 h-5 sm:w-6 sm:h-6" />
                  Pesca una Nuova Proposta
                </button>
          ...
          <div className="space-y-3 sm:space-y-4">
            {player.technologies.length === 0 ? (
              <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 text-center">
                ...
                <p className="text-gray-600 text-sm sm:text-base mb-2">Nessuna legge approvata ancora</p>
                <p className="text-gray-500 text-xs sm:text-sm">Presenta proposte per iniziare</p>
              </div>
            ) : (
              player.technologies.map((tech, index) => (
                <div
                  key={tech.id}
                  className="bg-white rounded-xl shadow-lg p-4 sm:p-5 hover:shadow-xl transition-shadow duration-200 border-l-4 border-blue-500"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="bg-blue-600 text-white text-xs sm:text-sm font-bold px-2 py-1 rounded">
                          #{index + 1}
                        </span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tech.techPoints > 0 && (
                      <span className="flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs sm:text-sm font-semibold">
                        <Microscope className="w-3 h-3 sm:w-4 sm:h-4" />
                        +{tech.techPoints} Tech
                      </span>
                    )}
                    {tech.ethicsPoints && tech.ethicsPoints > 0 && (
                      <span className="flex items-center gap-1 bg-green-100 text-green-800 px-2 py-1 rounded text-xs sm:text-sm font-semibold">
                        <Scale className="w-3 h-3 sm:w-4 sm:h-4" />
                        +{tech.ethicsPoints} Etica
                      </span>
                    )}
                    <span className="flex items-center gap-1 bg-purple-100 text-purple-800 px-2 py-1 rounded text-xs sm:text-sm font-semibold">
